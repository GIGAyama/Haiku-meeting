...
3. 左の「ファイル」の横にある「＋」から「HTML」を選び、これを4回くり返します。名前はそれぞれ app-shell、app、css、vendor にします。拡張子は自動で付くので入力しません。
4. それぞれに、リポジトリにある app-shell.html、app.html、css.html、vendor.html の中身を貼り付けます。
...
貼り付けるファイルは全部で5つです。app-shell が残りの3つを読み込む造りなので、1つでも欠けると画面が真っ白になります。名前が1文字でも違っても出ません。うまく出ないときは、まずここを疑ってください。

以前の版では、この外枠の名前は index でした。すでに index で貼ってお使いの場合は、そのままで動きます。貼り直すときに app-shell へ変えてください。
